...
- `getModelPrediction(...)`: Calls the local model HTTP endpoint on the configured localhost port, passes the neutral-venue flag, selects outcome by argmax over home/draw/away probabilities, records `predictedAt`, and maps results into `MatchInfo`.
...
- `createJsonlFileForModel(matches)` / `uploadJsonlToLocalModel(file)`: Helpers that serialize matches, including neutral-venue metadata and model metadata such as `predictedAt`, and perform the HTTP upload.
