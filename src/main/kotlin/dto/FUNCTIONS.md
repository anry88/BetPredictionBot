...
- `MatchInfo.kt`: Core match DTO exchanged between services, repositories, and bot formatting (ids, datetime, predicted/actual results, prediction timestamp, odds, neutral-venue marker, Telegram message references, and model probabilities).
- `JsonlMatch.kt`: Minimal match projection for exporting training data, neutral-venue metadata, and prediction metadata to the local model.
